 ...
 *
 * Fluxo em 3 etapas:
 *  1. Selecionar produtos (múltiplos diferentes, com quantidade)
 *  2. Preencher dados da cliente (todos opcionais, autocomplete de clientes cadastradas)
 *  3. Exibir QR Code + copia-e-cola + link compartilhável via WhatsApp
 */
...
export function AdminPixGerarModal({ onClose }: { onClose: () => void }) {
  ...
  const gerarPix = async () => {
    ...
    if (!clienteNome.trim()) {
      toast.error("Informe ao menos o nome da cliente.");
      return;
    }
    ...
    try {
      const result = await createPixPayment({
        customer: {
          ...
          // @ts-ignore — campo extra aceito pela edge function
  ...
  const pixLink = pixResult
    ? `${window.location.origin}/checkout/pix/${pixResult.order_id}`
    : "";
  ...
  const abrirWhatsApp = () => {
    ...
    const msg = encodeURIComponent(
      `Olá ${nome}! 🎀 Segue o link para pagar o seu pedido via Pix:\n${pixLink}\n\nValor: ${brl(pixResult!.total)}`
    );
    const base = phone
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-card rounded-3xl shadow-soft w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden animate-modal-in"
      >
        {/* ── Header ─────────────────────────────────────────────────────── */}
        ...
        {/* ── Step 1 — Produtos ─────────────────────────────────────────── */}
        {step === 1 && (
          <div className="flex flex-col md:flex-row gap-3 px-5 pb-5 overflow-hidden flex-1 min-h-0">
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              <p className="text-xs text-muted-foreground mb-2 shrink-0">
                Clique para adicionar ao carrinho — você pode selecionar produtos diferentes.
              </p>
              <div className="overflow-y-auto flex-1 space-y-1 pr-1">
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`flex items-center gap-3 p-2.5 rounded-xl border transition-colors cursor-pointer select-none ${
                        inCart
            ...
            {/* Carrinho lateral */}
            <div className="md:w-56 flex flex-col min-h-0 shrink-0">
              <div className="font-semibold text-sm mb-2 shrink-0 flex items-center gap-1.5">
                <QrCode className="h-4 w-4 text-primary" />
                Carrinho
                {cart.length > 0 && (
                  <span className="ml-auto text-xs bg-primary text-primary-foreground rounded-full px-1.5 py-0.5">
                    {cart.reduce((s, l) => s + l.quantity, 0)} itens
                  </span>
                )}
              </div>

              {cart.length === 0 ? (
                <div className="flex-1 flex items-center justify-center text-muted-foreground text-xs text-center border border-dashed border-border rounded-xl p-4">
                ...
                <div className="flex-1 overflow-y-auto space-y-1 pr-0.5 min-h-0">
                  {cart.map((l) => (
                    <div
                      key={l.productId}
                      className="flex items-center gap-2 bg-muted/40 border border-border rounded-xl p-2"
                    >
                      <img
                        src={l.image}
                        alt={l.name}
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        <button
                          ...
                          className="h-6 w-6 rounded-full bg-muted hover:bg-muted/70 flex items-center justify-center"
                        >
                        ...
                        <button
                          ...
                          className="h-6 w-6 rounded-full text-destructive hover:bg-destructive/10 flex items-center justify-center ml-0.5"
                        >
              ...
              {cart.length > 0 && (
                <div className="mt-2 shrink-0">
                  ...
                  <button
                    onClick={() => setStep(2)}
                    className="w-full h-10 rounded-full gradient-primary text-primary-foreground font-semibold text-sm flex items-center justify-center gap-1.5"
                  >
        ...
        {/* ── Step 2 — Dados da cliente ──────────────────────────────────── */}
        {step === 2 && (
          <div className="px-5 pb-5 overflow-y-auto flex-1">
            <p className="text-xs text-muted-foreground mb-4 bg-muted/40 rounded-xl px-3 py-2 border border-border">
              Todos os campos são <strong>opcionais</strong>. Se selecionar uma cliente cadastrada,
              o pedido será vinculado ao perfil dela automaticamente.
            </p>
            ...
            <div className="relative mb-3">
              ...
              <div className="relative">
                ...
                />
                {customerId && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5 font-semibold">
                    cadastrada
                  </div>
                )}
              </div>
              {sugestoes.length > 0 && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-card border border-border rounded-xl shadow-soft overflow-hidden">
                  {sugestoes.map((c) => (
                    ...
                    >
                      <div className="h-8 w-8 rounded-full gradient-primary text-primary-foreground grid place-items-center font-bold text-sm shrink-0">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                rows={2}
                placeholder="Algum detalhe especial sobre o pedido..."
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setStep(1)}
                className="h-11 px-4 rounded-full bg-muted font-semibold text-sm flex items-center gap-1.5 hover:bg-muted/70 transition-colors"
              >
                <ChevronLeft className="h-4 w-4" /> Voltar
              </button>
              <button
                onClick={gerarPix}
                disabled={generating || !clienteNome.trim()}
          ...
          <div className="px-5 pb-5 overflow-y-auto flex-1 space-y-4">
            ...
            <div className="bg-gradient-to-br from-primary to-rose-500 text-primary-foreground rounded-2xl p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm opacity-90">
                  Pedido{" "}
                  <span className="font-mono">#{pixResult.order_id.slice(0, 8)}</span> ·{" "}
                  {brl(pixResult.total)}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <button
                ...
                className={`mt-2 w-full h-10 rounded-full font-semibold text-sm flex items-center justify-center gap-2 transition-all ${
                  copied
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={copiarLink}
                  className="h-10 px-3 rounded-xl bg-muted border border-border hover:bg-muted/70 transition-colors"
                  title="Copiar link"
                >
            ...
            <button
              onClick={onClose}
              className="w-full h-11 rounded-full bg-muted font-semibold text-sm hover:bg-muted/70 transition-colors"
            >
              Fechar
            </button>
          </div>
