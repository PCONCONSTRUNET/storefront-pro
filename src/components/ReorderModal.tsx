...
export function ReorderModal({ order, onClose }: { order: Order; onClose: () => void }) {
  ...
  const toggle = (id: string) => setItems(arr => arr.map(i => i.productId === id && i.available ? { ...i, selected: !i.selected } : i));
  const setQty = (id: string, qty: number) => setItems(arr => arr.map(i => i.productId === id ? { ...i, qty: Math.max(1, Math.min(qty, i.maxStock || 1)) } : i));
      ...
      <div className="bg-card w-full sm:max-w-lg sm:rounded-2xl rounded-t-3xl shadow-soft max-h-[92vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Escolha o que recomprar do pedido #{order.id}</p>
          </div>
          <button onClick={onClose} aria-label="Fechar" className="p-2 hover:bg-muted rounded-full"><X className="h-5 w-5" /></button>
        </div>

        <div className="px-4 py-2 border-b border-border flex items-center justify-between text-xs">
          <button
            onClick={() => setItems(arr => arr.map(i => i.available ? { ...i, selected: !allOn } : i))}
            className="font-semibold text-primary hover:underline"
          >
            {allOn ? "Desmarcar todos" : "Selecionar todos"}
          </button>
          <span className="text-muted-foreground">{selected.length} de {items.filter(i => i.available).length} selecionado(s)</span>
        </div>

        <ul className="flex-1 overflow-y-auto divide-y divide-border">
          {items.map(it => (
            <li key={it.productId} className={`p-4 flex gap-3 ${!it.available ? "opacity-60" : ""}`}>
              <input
                type="checkbox"
                checked={it.selected}
              ...
              />
              <img src={it.image} alt={it.name} className="w-14 h-14 rounded-xl object-cover bg-muted shrink-0" />
              <div className="flex-1 min-w-0">
                ...
                {!it.available ? (
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-muted-foreground ml-1">Estoque: {it.maxStock}</span>
                  </div>
                )}
              </div>
              <div className="text-sm font-semibold whitespace-nowrap">{brl(it.price * it.qty)}</div>
            </li>
          ))}
        </ul>
