  ...
  return (
    <AdminLayout title="Categorias">
      <div className="flex justify-end mb-4">
        <button onClick={() => setEditing({ id: `cat_${Date.now()}`, name: "", image: "🎀", order: categories.length + 1 })}
        ...
        {categories.sort((a, b) => a.order - b.order).map(c => (
          <div key={c.id} className="bg-card rounded-2xl p-4 shadow-card text-center">
            ...
            <div className="flex gap-1 justify-center mt-3">
              ...
              <button onClick={() => { if (confirm("Excluir?")) { deleteCategory(c.id); toast.success("Excluída"); }}}
      ...
      {editing && (
        <Modal onClose={() => setEditing(null)} title="Categoria">
          <form onSubmit={e => { e.preventDefault(); upsertCategory(editing); toast.success("Salva!"); setEditing(null); }} className="space-y-3">
            <Field label="Nome" value={editing.name} onChange={v => setEditing({ ...editing, name: v })} />
            <Field label="Emoji / ícone" value={editing.image} onChange={v => setEditing({ ...editing, image: v })} />
            <Field label="Ordem" type="number" value={String(editing.order)} onChange={v => setEditing({ ...editing, order: parseInt(v) || 0 })} />
