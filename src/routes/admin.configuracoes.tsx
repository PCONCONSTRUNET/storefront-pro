...
function Page() {
  ...
  const save = () => { updateSettings(s); toast.success("Configurações salvas!"); };
    ...
    <AdminLayout title="Configurações da loja">
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card title="Entrega e pagamento">
          <Field label="Taxa de entrega" type="number" value={String(s.shippingFee)} onChange={v => setS({ ...s, shippingFee: parseFloat(v) || 0 })} />
          <Toggle label="Aceitar Pix" value={s.acceptPix} onChange={v => setS({ ...s, acceptPix: v })} />
          <Toggle label="Aceitar Cartão" value={s.acceptCard} onChange={v => setS({ ...s, acceptCard: v })} />
          <Toggle label="Aceitar Dinheiro na entrega" value={s.acceptCash} onChange={v => setS({ ...s, acceptCash: v })} />
        </Card>
      </div>
      <div className="mt-4 flex justify-end">
        <button onClick={save} className="px-6 h-11 rounded-full gradient-primary text-primary-foreground font-semibold">Salvar tudo</button>
      </div>
    </AdminLayout>
  );
}
...
function Toggle({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center justify-between gap-2 cursor-pointer">
      ...
      <button type="button" onClick={() => onChange(!value)} className={`w-11 h-6 rounded-full transition-colors relative ${value ? "bg-primary" : "bg-muted"}`}>
