...
function Page() {
  ...
  const finish = async () => {
    ...
    const order = placeOrder({
      ...
      address: settings.address, paymentMethod: form.payment,
      deliveryMethod: "retirada", notes: form.notes,
    });
    playBeep();
  ...
  const paymentOptions: { id: "pix" | "card" | "cash"; label: string; sub: string; icon: typeof QrCode; enabled: boolean }[] = [
    ...
    { id: "cash", label: "Dinheiro na retirada", sub: "Pague ao retirar no ateliê", icon: Banknote, enabled: settings.acceptCash },
  ];
          ...
          {step === 0 && (
            <div className="space-y-3">
              ...
              <div className="rounded-xl bg-accent/40 border border-accent p-3 text-sm mt-2">
                ...
                <div className="text-xs text-muted-foreground mt-1">Avisaremos pelo WhatsApp quando o pedido estiver pronto para retirada.</div>
              </div>
