...
function Page() {
  ...
  const [account, setAccount] = useState<{ subjectType: ResetSubject; subjectEmail: string } | null>(null);
  ...
  const loginPath = account?.subjectType === "admin" ? "/admin/login"
    : account?.subjectType === "affiliate" ? "/afiliada/login"
    : "/login";
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gradient-to-br from-background via-rose/30 to-accent p-4">
      <div className="w-full max-w-sm bg-card rounded-3xl shadow-soft p-6">
        ...
        {loading ? (
          <div className="mt-6 grid place-items-center"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div>
        ) : !token || !account ? (
          <div className="mt-6 text-center space-y-3">
            <p className="text-sm text-muted-foreground">Link inválido ou expirado.</p>
            <Link to="/esqueci-senha" className="inline-block text-sm text-primary underline">Solicitar novo link</Link>
          </div>
        ) : done ? (
