  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!account) {
        ...
        toast.error("Não encontramos essa conta neste dispositivo. A redefinição precisa ser feita no mesmo navegador onde a conta foi criada.");
        return;
      }
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gradient-to-br from-background via-rose/30 to-accent p-4">
      <div className="w-full max-w-sm bg-card rounded-3xl shadow-soft p-6">
        ...
        <p className="text-xs text-muted-foreground text-center mt-1">Digite seu e-mail e geramos um link seguro de redefinição.</p>
        ...
        {!link ? (
          ...
          <div className="mt-6 space-y-3">
            <div className="rounded-xl border border-border bg-background p-3">
              <p className="text-xs text-muted-foreground mb-1">Link gerado (válido por 30 minutos):</p>
              <p className="text-xs font-mono break-all text-foreground">{link}</p>
            </div>
            <button onClick={copyLink} className="w-full h-11 rounded-full bg-secondary text-secondary-foreground font-semibold inline-flex items-center justify-center gap-2">
              <Copy className="h-4 w-4" /> Copiar link
            </button>
            <button onClick={whatsappShare} className="w-full h-11 rounded-full bg-[#25D366] text-white font-semibold inline-flex items-center justify-center gap-2">
              <MessageCircle className="h-4 w-4" /> Enviar por WhatsApp
            </button>
            <button onClick={emailShare} className="w-full h-11 rounded-full bg-primary text-primary-foreground font-semibold inline-flex items-center justify-center gap-2">
