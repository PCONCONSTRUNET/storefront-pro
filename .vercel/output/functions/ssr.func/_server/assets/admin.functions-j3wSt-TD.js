...
const createConsignmentFn = createServerFn({
  ...
  const {
    data: result,
    error
  } = await supabase.rpc("admin_insert_consignment", {
    p_affiliate_id: data.affiliate_id,
    p_quantity: data.quantity,
    p_total_value: data.total_value,
    p_picked_up_at: data.picked_up_at ?? (/* @__PURE__ */ new Date()).toISOString(),
  ...
  if (!res?.ok) return {
    ok: false,
    message: res?.message || "Falha ao registrar"
  };
  return {
    ok: true,
    row: res.row
...
const createConsignmentWithNewAffiliateFn_createServerFn_handler = createServerRpc({
  id: "a5de0762612a5290d103d8cc2722d93e83ccacd25fdf9a81983f3c8488050bbc",
  name: "createConsignmentWithNewAffiliateFn",
  filename: "src/lib/admin.functions.ts"
}, (opts) => createConsignmentWithNewAffiliateFn.__executeServer(opts));
const createConsignmentWithNewAffiliateFn = createServerFn({
  ...
  picked_up_at: stringType().datetime().optional(),
  notes: stringType().trim().max(1e3).optional()
}).parse(i)).handler(createConsignmentWithNewAffiliateFn_createServerFn_handler, async ({
  data
}) => {
  await requireAdminAuth();
  const {
    data: result,
    error
  } = await supabase.rpc("admin_create_affiliate_consignment", {
...
export {
  ...
  createConsignmentWithNewAffiliateFn_createServerFn_handler,
